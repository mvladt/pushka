...
- [x] nginx: новый `/etc/nginx/conf.d/pushka.conf` (`server_name pushka.mvladt.ru`,
      ...
      отдельным файлом, старый `webpush-scheduler.conf` не тронут — оба домена сейчас ведут на
      один и тот же бэкенд `127.0.0.1:3002` (сервис/systemd-юнит/пользователь ещё старые).
