...
- `npm run test:playwright` — 1/1. Важно было прогнать отдельно: автофикс переформатировал
  `src/client/index.html` (инлайн-скрипт) и `sw.js`, а интеграционный тест проверяет только что
  файл отдаётся, не что скрипт реально исполняется.
