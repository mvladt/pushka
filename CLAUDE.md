...
Pushka — планировщик Web Push уведомлений на Node.js и Express. Принимает push-подписки с временем отправки, сохраняет и отправляет через Web Push Protocol.
...
Деплой и состояние прод-сервера описаны в `docs/archive/cicd-plan.md` и `docs/archive/cicd-result.md`. Общее администрирование сервера (сервер целиком, фаервол, другие сервисы на нём) ведётся в соседнем проекте `server-management`, который лежит (скорее всего) в `~/Projects/MyOwn/`.

## Журнал решений

Помимо стандартных `_-plan.md`/`_-task.md`/`_-result.md` (см. глобальный `CLAUDE.md`), в
`docs/decisions.md` ведётся лёгкий сквозной журнал значимых решений проекта: дата, что решили,
почему, ссылка на полный план/результат за подробностями. Не дублирует пошаговые доки — даёт
быстрый ответ на «что и почему решили», не заставляя перебирать архив.
